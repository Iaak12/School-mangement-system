...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const parents = await Parent.find(query)
    ...
    .skip((page - 1) * limit)
    .limit(Number(limit));
  return res.status(200).json(new ApiResponse(200, { parents, pagination: { total, page: Number(page) } }));
}));

router.get('/:id', asyncHandler(async (req, res) => {
  const parent = await Parent.findById(req.params.id)
    .populate({ path: 'students', populate: [{ path: 'class', select: 'name' }, { path: 'section', select: 'name' }] });
...
router.post('/', authorize('admin', 'principal'), asyncHandler(async (req, res) => {
  ...
  const parent = await Parent.create({ ...parentData, user: user._id });
  await User.findByIdAndUpdate(user._id, { profileRef: parent._id, profileModel: 'Parent' });
  return res.status(201).json(new ApiResponse(201, parent, 'Parent created.'));
}));
